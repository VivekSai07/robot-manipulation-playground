...
def main():
    ...
    # Mark-1 IK uses arm_dof
    ik = IKController(robot, arm_dof=ARM_DOF, kp_pos=5.0, kp_rot=3.0)
    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:
        ...
        while viewer.is_running():
            ...
            if current_state_idx < len(states):
                ...
                # M1 controller doesn't use q_posture (nullspace)
                dq, err, done = ik.compute_velocity(q_current, target_se3)

                # 2. Velocity to Position Integration
                max_speed = 1.0
                dq = np.clip(dq, -max_speed, max_speed)
                
                # CRITICAL FIX: Use Franka's PD Position Actuators!
