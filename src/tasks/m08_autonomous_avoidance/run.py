...
SCENE_PATH = os.path.join(ROBOT_DIR, "model", "m8_scene.xml")
...
def main():
    parser = argparse.ArgumentParser(description="Mark-8 Autonomous Avoidance")
    ...
    parser.add_argument("--debug", action="store_true",
                        help="Show segmentation debug view during scan")
    ...
    # Perception Stow: rotate base 90° to clear camera view before scattering
    d.qpos[0] = -1.57
    ...
    for color in ["red", "green", "blue", "yellow"]:
        try:
            ...
            for _ in range(50):
                # Keep cubes well on the pick side of the wall (wall face at y=0.23,
                # RRT clearance=0.04 extends boundary to y=0.19 — stay below y=0.05)
                px = np.random.uniform(0.35, 0.55)
                py = np.random.uniform(-0.20, 0.05)
                ...
                if state["name"] == "Perception Scan":
                    ...
                    if t_state > state["duration"]:
                        ...
                        if estimated_pos is not None:
                            ...
                            PLACE = np.array([0.5, 0.5, 0.02 + GRIPPER_Z_OFFSET])
